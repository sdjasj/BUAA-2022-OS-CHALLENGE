...
usr_load_elf(int fd , Elf32_Phdr *ph, int child_envid){
	...
	if (r) return r;
    if (offset) {
		r = syscall_mem_alloc(child_envid, va, PTE_R | PTE_V);
		if (r) return r;
		r = syscall_mem_map(child_envid, va, 0, tmp, PTE_R | PTE_V);
		if (r) return r;
        u_long size = MIN(bin_size, BY2PG - offset);
        user_bcopy((void*)blk, (void*)(tmp + offset), size);
    ...
    * Step 1: load all content of bin into memory. *
    for (; i < bin_size;) {
      * Hint: You should alloc a new page. *
        ...
        if (r) return r;
        r = syscall_mem_map(child_envid, va + i, 0, tmp, PTE_R | PTE_V);
        if (r) return r;
        u_long size = MIN(bin_size - i, BY2PG);
        user_bcopy((void*)(blk + i), (void*)tmp, size);
        r = syscall_mem_unmap(0, tmp);
        if (r) return r;
        i += size;
    }
    * Step 2: alloc pages to reach `sgsize` when `bin_size` < `sgsize`.
    ...
    i = bin_size;
    u_long curva = va + i;
    offset = curva - ROUNDDOWN(curva, BY2PG);
	i = i + MIN(sgsize - i, BY2PG - offset);;
	*
    if (offset) {
        r = syscall_mem_alloc(child_envid, va + i, PTE_R | PTE_V);
        if (r) return r;
        ...
        if (r) return r;
        u_long size = MIN(bin_size, BY2PG - offset);
        user_bcopy((void*)(blk + i), (void*)(tmp + offset), size);
        r = syscall_mem_unmap(0, tmp);
        if (r) return r;
	...
	*
    while (i < sgsize) {
        ...
        r = syscall_mem_map(child_envid, va + i, 0, tmp, PTE_R | PTE_V);
        if (r) return r;
        u_long size = MIN(sgsize - i, BY2PG);
        user_bcopy((void*)(blk + i), (void*)tmp, size);
        r = syscall_mem_unmap(0, tmp);
        if (r) return r;
        i += size;
...
int spawn(char *prog, char **argv)
{
	...
	int fd;
	u_int child_envid;
	int size, text_start, entry_size, count;
	u_int i, *blk;
	u_int esp;
	Elf32_Ehdr* elf;
	//Elf32_Ehdr *ehdr;
	...
	if((r=open(pprog, O_RDONLY))<0){
		//user_panic("spawn ::open line 102 RDONLY wrong !\n");
		return r;
	}
	// Your code begins here
	// Before Step 2 , You had better check the "target" spawned is a execute bin 
	fd = r; //open return fdnum
    elfbuf = INDEX2DATA(fd);
//	r = readn(fd, elfbuf, sizeof(Elf32_Ehdr));
	if (r < 0) user_panic("read elf error in spawn\n"); 
	if (!usr_is_elf_format(elfbuf)){
		user_panic("file is not elf");
	}
	// Step 2: Allocate an env (Hint: using syscall_env_alloc())
	child_envid = syscall_env_alloc();
	if (child_envid < 0) user_panic("error in chile_envid: %d\n",child_envid);
	...
	ph = elfbuf + elf->e_phoff; // char offset
