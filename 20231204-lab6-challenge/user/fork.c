...
pgfault(u_int va)
{
	...
	perm -= PTE_COW;
	tmp = USTACKTOP;
	ret = syscall_mem_alloc(0, tmp, perm);
	u_int roundva = ROUNDDOWN(va, BY2PG);
	user_bcopy((void*)roundva, (void*)tmp, BY2PG);
...
duppage(u_int envid, u_int pn)
{
	...
	int flag = 0;
	if (perm & PTE_R) {
		if (!(perm & PTE_LIBRARY) && !(perm & PTE_FORK)) {
			flag = 1;
			perm |= PTE_COW;
		}	
	}	
...
fork(void)
{
	...
	if (newenvid != 0) {
		for (i = 0; i < VPN(USTACKTOP); i++){
			if (((*vpd)[i >> 10] & PTE_V) != 0 && ((*vpt)[i] & PTE_V) != 0)
				duppage(newenvid, i);
		}
		int r = syscall_mem_alloc(newenvid, UXSTACKTOP - BY2PG, PTE_V | PTE_R);
