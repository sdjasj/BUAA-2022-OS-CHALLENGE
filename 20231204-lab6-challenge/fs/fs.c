...
diskaddr(u_int blockno)
{
	if (super && blockno > super->s_nblocks) {
		user_panic("error in diskaddr: `blockno` is greater than nblocks\n");
	}
...
unmap_block(u_int blockno)
{
	...
	if (r) user_panic("error in unmap_block\n");
...
free_block(u_int blockno)
{
	...
	if (super == 0 || blockno == 0 || blockno >= super->s_nblocks){
		user_panic("Invalid blockno in free_block!\n");
	}
	// Step 2: Update the flag bit in bitmap.
	bitmap[blockno >> 5] |= (1 << (blockno % 32));
